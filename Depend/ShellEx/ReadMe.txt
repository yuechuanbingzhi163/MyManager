...
应用程序向导已为您创建了此 ShellEx 项目，作为编写动态链接库 (DLL) 
的起点。
...
ShellEx.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(ShellEx.h)
        GUID 定义			(ShellEx_i.c)
        类型库				(ShellEx.tlb)
        封送处理代码			(ShellEx_p.c 和 
                                           dlldata.c）

ShellEx.h
    此文件包含 ShellEx.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 ShellEx.pch 的预编译头 (PCH) 文件和
...
ShellExps.vcxproj
    ...
    文件。此进程生成 dlldata.c、ShellEx_i.c 和 
    ShellEx_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

ShellExps.vcxproj.filters
