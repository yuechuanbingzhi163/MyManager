...
{
	static	ULONG		ulNum	= 0;

	ZeroMemory(ms_TempName, sizeof(ms_TempName));
	wsprintf(ms_TempName, L"bs_%d", ulNum++);
	return ms_TempName;
}
...
BOOL CBinStream::SplitFile(IN PWSTR lpFileName, IN LPFRAG_INFO_T pData)
{
	...
	LARGE_INTEGER		liTmp, liSegment;

	// 新处理方式
	PVOID				pMain		= NULL;
	PVOID				pSub[128]	= {0};									// 预置128个指针空间
	DWORD				dwCount		= 0;
	DWORD				dwPos		= {0};
	DWORD				dwBits		= 0;			// 仅支持 4GB / 8 的文件大小，即512MB
	...
	// 基于二进制流打开原始文件
	if(!Gen(FILE_STREAM, &liBinSize, lpFileName, OPEN_BIN_STREAM, &BinStream))
	{
		return FALSE;
	}
	...
	// 设置访问指针
	for(DWORD i = 0; i < pData->Count; i++)
		pSub[i] = BinStreamSub[i].DataAddr;

	// 说明：
	//			i = 总数， j = 每一片当前数
	//			m = 当前分片
// 	for(DWORD i = 0, j = 0; i < dwBits; j++)
// 	{
// 		for(int m = 0; m < dwCount && i < dwBits; m++, i++)
// 		{
// 			__asm
// 			{
// 				mov		eax, i;
...
// 	}
	...
	// 设置环境
	pMain = BinStream.DataAddr;
	dwCount = pData->Count;
	dwBits = BinStream.liBinSize.LowPart;
	...
	__asm
	{
		...
		xor		eax, eax;			// 用于总数
		xor		ebx, ebx;			// 用于每片当前数
		mov		edx, dwCount;		// 用于分片数
		mov		esi, pMain;			// 用于原始文件
