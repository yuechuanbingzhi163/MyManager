...
CDropTargetEx::CDropTargetEx(CWndMainFrame *pMainWnd):
	m_pMainWnd(pMainWnd),
	tb_RefCount(0),
	m_hTargetWnd(0),
	m_AcceptKeyState(0),
...
BOOL CDropTargetEx::DragDropRegister( HWND hWnd,DWORD AcceptKeyState /*= MK_LBUTTON*/ )
{
	...
	{ 		
		...
		if (m_pMainWnd->GetFileListPos(m_rcList))
			return true;
		return false;
	}
	else 
...
HRESULT STDMETHODCALLTYPE CDropTargetEx::DragOver(DWORD grfKeyState,POINTL pt,	DWORD *pdwEffect)
{
	...
	if( grfKeyState != m_AcceptKeyState || !(m_pMainWnd->IsDropValidRect(pt)))
	{
...
HRESULT STDMETHODCALLTYPE CDropTargetEx::Drop(IDataObject *pDataObj,DWORD grfKeyState,	POINTL pt,	DWORD __RPC_FAR *pdwEffect)
{
	...
	for(UINT i = 0; i < m_Array.size(); i++)
	{
		...
		tempt.x = pt.x;
		tempt.y = pt.y;
		::ScreenToClient(m_pMainWnd->GetHWND(), &tempt);
...
void CDropTargetEx::ProcessDrop(LPDRAGDATA pDropData/*HDROP hDrop*/, POINT pt)
{
	...
	case CF_HDROP:
		{
			m_pMainWnd->OnDropFiles(m_pMainWnd, (HDROP)pDropData->stgMedium.hGlobal, pt);
			break;
		}
	default:
		break;
